<template>
  <div class="main">
    ...
    <form @submit.prevent="onSubmit" novalidate>
      ...
      <div class="mb-3">
        <label class="form-label"> Multiple User Specific Field ID </label>
        <input
          type="text"
          class="form-control"
...
export default {
  ...
  computed: {
    ...
    businessRuleNameSet() {
      const list = Array.isArray(this.filteredRules) ? this.filteredRules : [];
      return list.map((rule) => (rule.name || "").trim().toLowerCase());
    },

    dataModelOptions() {
      console.log("HEy", this.filteredModel);
    ...
    destinationOptions() {
      ...
      // You can add more destination types here (applications, reports, etc.)
      return [];
    },
  },
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        const payload = {
          ...
          user_specific_field_id: this.form.user_specific_field_id || null,
          multiple_user_specific_field_id:
